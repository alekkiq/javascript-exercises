...
picArray.forEach((article) => {
  ...
  // create the h2 & append it to the article
  const heading = document.createElement('h2');
  heading.textContent = article.title;

  // create the figure (img & figcap)
  const figure = document.createElement('figure');

  const imgElement = document.createElement('img');
  imgElement.src = article.image.medium;
  ...
  // append all created elements to the article element
  articleElement.appendChild(heading);
  articleElement.appendChild(figure);
  articleElement.appendChild(paragraph);

  // ... and append the article to the target
  target.appendChild(articleElement)
});